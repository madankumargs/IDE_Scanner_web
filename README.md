...
SARVAM_REASONING_MODEL=glm5.3-flash # optional allowlisted override
```

The interactive default is `glm5.3-flash` for lower-latency global report generation. `sarvam-105b` remains available as an explicit override, while `deepseekv4-flash`, `glm5.2`, `glm5.3`, and `gemma4` are allowlisted for experiments through Sarvam's `/v2` endpoint; beta models may require access for the specific key. Reasoning traces returned by beta models are intentionally discarded; only the validated report is shown. The endpoint requires authentication, limits each user/release to three requests per ten minutes in the shared D1 usage table, uses a 20-second timeout, and returns `private, no-store` responses.
