...
The default is `sarvam-105b` for the standard-key interactive report path. `deepseekv4-flash`, `glm5.2`, `glm5.3`, `glm5.3-flash`, and `gemma4` are allowlisted for operator experiments through Sarvam's `/v2` endpoint; beta models may require access for the specific key. Reasoning traces returned by beta models are intentionally discarded; only the validated report is shown. The endpoint requires authentication, limits each user to three requests per ten minutes in the shared D1 usage table, uses a 28-second intelligence timeout, and returns `private, no-store` responses.
