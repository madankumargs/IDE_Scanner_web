...
const candidateScanRequested = String(process.env.CATALOG_CANDIDATE_SCAN_ENABLED || "").trim().toLowerCase() === "true";
if (candidateScanRequested && !/^[0-9a-f]{40}$/i.test(scannerBuild)) {
  throw new Error("Candidate scans require a full 40-character scanner build SHA.");
}
...
if (bulkScanRequested && !bulkCatalogReady) {
  throw new Error(candidateScanRequested
    ? "Candidate scan was requested, but its scanner build is not valid."
    : "Bulk scan was requested, but no active accuracy-attested release matches the scanner build. Activate the validated release before retrying.");
}
