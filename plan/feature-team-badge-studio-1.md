...
| ID | Task | Files / scope | Depends on | Completion criteria | Status |
| --- | --- | --- | --- | --- | --- |
| TASK-007 | Add Badge Health to Overview and Extensions. Show ready/current, pending, stale, drifted, failed, and uncovered watched extensions; include “refresh” and “create badge” actions. | `app/TeamWorkspace.tsx`; `app/workspace/views/OverviewView.tsx` if present; `app/workspace/views/ExtensionsView.tsx`; shared workspace types/styles | Phase 1 badge list; existing watchlist and overview data | A team can understand fleet badge coverage and act on the highest-value item without opening every extension. | Partial |
| TASK-008 | Connect release event ingestion to badge state. On a new observed release, mark exact badges as drifted/stale, preserve the old badge, and offer a one-click scan/refresh for the new version. | Existing release-event route/queue and `lib/cloudflareWorkspace.ts`; `lib/teamBadges.ts`; badge API | Existing `release_events`, `ReleaseEventQueue`, watchlist baseline logic | Release event tests verify idempotency, no silent public mutation, and correct next-action state. | Partial |
| TASK-009 | Add evidence-aware change summaries to badge detail and Activity. Reuse scan report facts and existing evidence/decision surfaces to show changed capabilities, score delta, or review status. | `app/workspace/views/ActivityView.tsx`; badge detail UI; existing evidence intelligence/report surfaces where appropriate | Existing report JSON, activity/audit data, Phase 2 drift state | The user can explain “what changed since last badge” with a report link and concise evidence, not just a new number. | Not started |
| TASK-010 | Add a weekly digest and in-product reminders only for high-signal badge events: new release, score change, failed refresh, or badge nearing staleness. Respect existing workspace preferences and delivery channels. | Existing digest/notification routes and preferences; new badge event mapping | Existing team digest, channels, deliveries, notification preferences | Digest delivery is opt-in/controlled by current preferences, deduplicated, auditable, and suppressible. | Not started |
| TASK-011 | Add explicit public publishing controls and a lightweight public Trust Card/Badge Wall experiment. Allow a team to choose which exact badges are public, preview the sanitized output, revoke publication, and link back to public reports. | New public route such as `app/api/team-badges/[token].svg/route.ts` or a canonical equivalent; workspace publish UI | Privacy projection, role checks, badge history | Public pages contain no team-private data and clearly distinguish exact-release facts from live status. | Partial |
...
| ID | Task | Files / scope | Depends on | Completion criteria | Status |
| --- | --- | --- | --- | --- | --- |
| TASK-012 | Add fleet-level export for selected inventory/watchlist items: Markdown badge snippets, a `GUARDRAILS.md` trust inventory, and JSON for automation. | New workspace export endpoint/component; Badge Studio export UI | Stable badge records and privacy rules | A team can update its repository trust documentation in one repeatable action, with exact release identity preserved. | Not started |
| TASK-013 | Add optional PR/status-check and webhook integrations using existing notification/channel primitives. Start with a release-change summary and badge refresh recommendation rather than automatic public publishing. | Existing webhook/channel delivery code; new integration adapter and settings UI | Phase 2 event model; existing team API/integration patterns | Integrations are opt-in, replay-safe, scoped to a team, and do not leak private scan content. | Not started |
| TASK-014 | Add usage limits, scan budget protections, and entitlement hooks. Make repeated refresh requests safe and explain limits in the UI. | Existing billing/entitlement surfaces; badge/deep-scan admission checks | Current plan/billing model and scan lifecycle | A team cannot accidentally create an unbounded scan loop; blocked actions explain how to proceed. | Not started |
