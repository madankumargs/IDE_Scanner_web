...
- **CON-003**: Use the existing Sarvam allowlist and structured-output controls in `lib/sarvam.ts`. `sarvam-105b` remains the default for standard-key availability; v2 models remain allowlisted for explicit operator overrides and must retain the current thinking-disable controls for structured output.
