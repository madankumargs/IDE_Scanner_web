...
- **CON-002**: Preserve the existing Evidence Review Brief endpoint and UI behavior while adding the richer intelligence report as a versioned capability or a separate endpoint. Existing production users must not receive a breaking response shape.
- **CON-003**: Use the existing Sarvam allowlist and structured-output controls in `lib/sarvam.ts`. `sarvam-105b` remains the default primary model; v2 reasoning models must retain the current thinking-disable controls for structured output.
